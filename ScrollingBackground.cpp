...
void BackgroundLayer::Update()
{
  ...
  case SD_UP:
    ...
    if (m_rcViewport.bottom >715)
    {
      m_rcViewport.bottom = 715;
      m_rcViewport.top = 715-256;
    }
    break;
  ...
  case SD_RIGHT:
    ...
    if (m_rcViewport.left < 0)
    {
      m_rcViewport.left = 0;
      m_rcViewport.right = 256;
    }
    break;
  ...
  case SD_LEFT:
    ...
    m_rcViewport.left += m_iSpeed;
    m_rcViewport.right += m_iSpeed;
    if (m_rcViewport.right > 1154)
    {
      m_rcViewport.right = 1154;
      m_rcViewport.left = 1154 - 256;
    }
    break;
  }
}
